    ...
    private http: HttpClient) {
     ...
     onFileChange(files: FileList) {
      if (files && files.length > 0) {
        ...
        this.bookcode.setValue(file.name);
      }
    }
  
    onSubmit(files: FileList) {
      const formData = new FormData();
      formData.append('bookcode', files[0]);
  
      this.loading = true;
      // Send data (payload = formData)
      console.log(formData.get('bookcode'));
  
      // 폼데이터를 서버로 전송한다.
      this.http.post(`${apiurl}/gcUnit/upload/bookcode`, formData)
        .subscribe(res => {
          this.result = res;
          this.loading = false;
          this.bookcode.setValue(null);
        });
    }
  
    get bookcode() {
      return this.form.get('bookcode');
    }
